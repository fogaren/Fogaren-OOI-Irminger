% Code builds off Hilary's reading in of my calibrated oxygen cruise
% products and Bottle Summaries 
...
addpath('G:\My Drive\Matlab_work\BC\Irminger\OOI_Irminger_students\common')
...
%     U_deep = nanmean(abs(btlsum{yr}.Winkler_umolkg(deep) - btlsum{yr}.DOcorr_umolkg(deep)));  
    ...
    figure(100*yr + 1); clf
    subplot(2,2,1)
    plot(btlsum{yr}.prs, (btlsum{yr}.Winkler_umolkg - btlsum{yr}.DOcorr_umolkg), '.k','MarkerSize',10); hold on;
%     plot([0 3000], 0*[0 3000], 'k--')
    ...
    subplot(2,2,2)
    plot(btlsum{yr}.Date, (btlsum{yr}.Winkler_umolkg - btlsum{yr}.DOcorr_umolkg), '.k','MarkerSize',10); hold on;
    ylabel('Residual, Winkler - SBE O_2, \mumol/kg')
    xlabel('Cast Time')
    ylim([-10 10])
    grid on
    ...
    subplot(2,2,3)
    plot(btlsum{yr}.t, (btlsum{yr}.Winkler_umolkg - btlsum{yr}.DOcorr_umolkg), '.k','MarkerSize',10); hold on;
    ylabel('Residual, Winkler - SBE O_2, \mumol/kg')
    ...
    subplot(2,2,4)
    plot(btlsum{yr}.DOcorr_umolkg, (btlsum{yr}.Winkler_umolkg - btlsum{yr}.DOcorr_umolkg), '.k','MarkerSize',10); hold on;
    ylabel('Residual, Winkler - SBE O_2, \mumol/kg')
    xlabel('Winkler (\mumol/kg)')
    ylim([-10 10])
    grid on
    sgtitle(['Irminger Year ' num2str(yr) ': My corrections'])
end
yr = [1:5,8,9];
                  for i = 1:length(yr)  
            figure(80)
            subplot(2,2,1)
                    ...
                    plot(btlsum{yr(i)}.prs, (btlsum{yr(i)}.Winkler_umolkg - btlsum{yr(i)}.DOcorr_umolkg), '.','MarkerSize',10); hold on;
            ylabel('Residual, Winkler - SBE O_2, \mumol/kg')
            xlabel('Pressure (db)')
            ylim([-10 10])
            grid on
            ...
            %Plot residuals versus cast number 
            subplot(2,2,2)
            plot(btlsum{yr(i)}.Cast, (btlsum{yr(i)}.Winkler_umolkg - btlsum{yr(i)}.DOcorr_umolkg), '.','MarkerSize',10); hold on;
            
        %     plot([0 max(btlsum{yr}.Cast)], 0*[0 max(btlsum{yr}.Cast)], 'k--')
            ylabel('Residual, Winkler - SBE O_2, \mumol/kg')
            xlabel('Cast Number')
            ylim([-10 10])
            grid on
            ...
            %Plot residuals versus temperature 
            subplot(2,2,3)
            plot(btlsum{yr(i)}.pt, (btlsum{yr(i)}.Winkler_umolkg - btlsum{yr(i)}.DOcorr_umolkg), '.','MarkerSize',10); hold on;
            ylabel('Residual, Winkler - SBE O_2, \mumol/kg')
            xlabel('Temperature (\circC)')
            ylim([-10 10])
            grid on
            ...
            %Plot residuals versus oxygen concentration 
            subplot(2,2,4)
            plot(btlsum{yr(i)}.DOcorr_umolkg, (btlsum{yr(i)}.Winkler_umolkg - btlsum{yr(i)}.DOcorr_umolkg), '.','MarkerSize',10); hold on;
            ylabel('Residual, Winkler - SBE O_2, \mumol/kg')
            xlabel('Winkler (\mumol/kg)')
            ylim([-10 10])
            grid on
            sgtitle(['Irminger All Years: My corrections'])
                  end

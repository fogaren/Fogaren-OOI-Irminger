%% Load workspace 
% clearvars; close all
cd('G:\Shared drives\NSF_Irminger\Data_Files\From_Hilary\CalibratedOxygenProduct_Sept2023') % Pc = 0.032
cd('G:\Shared drives\NSF_Irminger\Data_Files\From_Hilary\CalibratedOxygenProduct_Jan2024') % Pc = 0.04
...
load('wfpmerge_output_variablePc1600db.mat')
...
wfP_DOresp_rate_umolkg_day = wfP_DOresp_rate_umolkg_day;
wfp_DOresp_rate_umolkg_day_95CI_high = wfp_DOresp_rate_umolkg_day_95CI_high;
wfp_DOresp_rate_umolkg_day_95CI_low = wfp_DOresp_rate_umolkg_day_95CI_low; 
wfp_b_umolkg = wfp_b_umolkg;
wfp_p_value = [];
wfp_R2 = [];
wfp_regress_days = []; 
wfp_DOresp_season_umolkg = []; % rate (slope) *resp_days 

...
[ind10,~] = find(btlsum.Cast == 10);
[ind21,~] = find(btlsum.Cast == 21);
Winklers = btlsum.Winkler_umolkg; % umol/kg calculated using calibrated CTD data 
Winklers(ind10) = NaN; % Removes data associated with unique glider casts
Winklers(ind21) = NaN; % 
...
mdl = mdl7;
Winkler_outliers = Winkler_outliers6;
...
btlsum.dt = datenum(btlsum.Date) - datenum(2018,06,08,19,02,37); % minus first cast time
...
% cal for SOC_dt 
cal.SOCcalc_dt = mdl_dt4.Coefficients.Estimate(1);
cal.Ecalc_dt = mdl_dt4.Coefficients.Estimate(2);
cal.SOCrate_dt = mdl_dt4.Coefficients.Estimate(3);
...
cal.SOCcalc10 = mdl.Coefficients.Estimate(1);
cal.gain_calc10 = cal.SOCcalc10/cal.SOC;
...
[ind21,~] = find(btlsum.Cast ~= 21);
Winklers = btlsum.Winkler_umolkg; % umol/kg calculated using calibrated CTD data 
Winklers(ind21) = NaN; % Removes data not associated with unique glider casts
...
cal.SOCcalc21 = mdl.Coefficients.Estimate(1);
cal.gain_calc21 = cal.SOCcalc21/cal.SOC;
...
xlabel('Winkler (\mumol/kg)')
grid on
sgtitle('Irminger Year 5 Cast 21: NLMR')

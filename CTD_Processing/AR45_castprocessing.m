...
AA277.D = 0.032; %0.027; % Default is 0.032
...
% Calculate AA gain
AA277.gain = nanmean(btlsum_tbl.AA_gain(btlsum_tbl.prs > 500))
AA277.gainstd = nanstd(btlsum_tbl.AA_gain(btlsum_tbl.prs > 500))
...
f.Position = [100 100 840 500];
    subplot(2,2,1)
    plot(btlsum{btl_num(i)}.prs,btlsum{btl_num(i)}.Winkler_umolkg - btlsum{btl_num(i)}.AA_DOcorr_umolkg,'.k','Markersize',20)
    hold on; grid on
ylabel({'Residual, Winkler - Aanderaa','(\mumol/kg)'}); xlabel('Pressure (db)')

    subplot(2,2,2)
    plot(btlsum{btl_num(i)}.Cast,btlsum{btl_num(i)}.Winkler_umolkg - btlsum{btl_num(i)}.AA_DOcorr_umolkg,'.k','Markersize',20)
    hold on; grid on
...
ylabel({'Residual, Winkler - Aanderaa','(\mumol/kg)'}); xlabel('Temperature (\circC)')
    ...
    subplot(2,2,4)
    plot(btlsum{btl_num(i)}.Winkler_umolkg,btlsum{btl_num(i)}.Winkler_umolkg - btlsum{btl_num(i)}.AA_DOcorr_umolkg,'.k','Markersize',20)
    hold on; grid on
...
function cast = process_cast(leah_cast,mycast,CTD_sen,cal,SOC_type,CruiseStartTime)
    ...
    vars = {'Station','prs','depth','lat','lon','temp1','temp2','sal1','sal2','CastTimeS','CastTimeUTC','StartTimeUTC','cruise_d','CTDcal',...
        'CTD_sen','t','CT','pt','SP','SA','rho','prho','sigma0','oxy_volts','DOcorr_umolkg','O2sol_umolkg','SOC_type','AA_volts'};
    cast = cast(:,vars); 
...
%     Calculate uncalibrated oxygen concentration for upcasts 
    ...
    vars = {'Station','prs','depth','lat','lon','temp1','temp2','sal1','sal2','CastTimeS','CastTimeUTC','StartTimeUTC','CTDcal',...
        ...
        'AA_volts','AA_volts_lagcorr','AA_volts_lagcorr_sm','AA_calphase','AA_spcorr_umolkg'};
    ...
    castd = castd(:,vars); 
      ...
      % Calculate gain from Winkler over Aanderaa lag/sal/press corrected values
            btlsum.AA_gain = btlsum.Winkler_umolkg./btlsum.AA_spcorr_umolkg;

                 % Reorder variables and remove unnecessary one        
        btlvars = {'Cruise','Date','Cast','Bottle','prs','depth','lat','lon','temp1','temp2','sal1','sal2','Discrete_Salinity_psu',...
            'CTDcal','CTD_sen','t','CT','pt','SP','SA','rho','prho','sigma0','oxy_volts','Winkler_mLL','Winkler_umolkg','O2sol_umolkg',...
            'SOC_type','DOcorr_umolkg','NLMR_Outlier','AA_volts','AA_spcorr_umolkg','AA_gain'};
        btlsum = btlsum(:,btlvars);
        end
...
for i = 1:length(cast_num)
    ...
    if max(cast_num(i) == bad_casts(:,1)) ~=1
        plot(DOuncorr_umolkg(13:end),downcasts{cast_num(i)}.pt(13:end),'Linewidth',1,'Color',grey)
        hold on
    end
end
...
title('Upcasts')
grid on
sgtitle([btlsum_tbl.Cruise(1) 'SBE 43 Oxygen'])
...
title('Upcasts')
grid on
sgtitle([btlsum_tbl.Cruise(1) 'Aanderaa Oxygen'])
legend('Uncalibrated','Calibrated','Winklers Removed','Winklers Used','Location','NW')

...
btl.Winkler_OOI_mLL = double(btl.Winkler_OOI_mLL);
...
btlsum = [btlsum05; btlsum06; btlsum07; btlsum15];
% btlsum10 and btlsum21 are processed independently 
...
%% =======================================================================
% Same oxygen sensor for whole cruise 
 % For all casts except 10 and 21 
...
% 
Winklers = btlsum.Winkler_umolkg; % umol/kg calculated using calibrated CTD data % average HIP/OOI
Winklers = btlsum.Winkler_OOI_umolkg; % umol/kg calculated using calibrated CTD data 
Winklers = btlsum.Winkler_HIP_umolkg; % umol/kg calculated using calibrated CTD data 
...
xlabel('Winkler (\mumol/kg)')
grid on
sgtitle('Irminger Year 5: SOC_d_t')
...
xlabel('Winkler (\mumol/kg)')
grid on
sgtitle('Irminger Year 5: SOC_c_n')
...
%Plot residuals versus pressure 
subplot(2,2,1)
plot(btlsum.prs, mdlcal10.Residuals.raw, 'k.','Markersize',10); hold on;
ylabel({'Residual, Winkler - NLMR output','(\mumol/kg)'})
xlabel('Pressure (db)')
grid on
...
%Plot residuals versus cast number 
subplot(2,2,2)
plot(datenum(btlsum.Date) - min(datenum(btlsum.Date)), mdlcal10.Residuals.raw, 'k.','Markersize',10); hold on;
ylabel({'Residual, Winkler - NLMR output','(\mumol/kg)'})
xlabel(['Days since ' datestr(min(datenum(btlsum.Date)))])
grid on
...
%Plot residuals versus temperature 
subplot(2,2,3)
plot(btlsum.t, mdlcal10.Residuals.raw, 'k.','Markersize',10); hold on;
ylabel({'Residual, Winkler - NLMR output','(\mumol/kg)'})
xlabel('Temperature (\circC)')
grid on
...
%Plot residuals versus oxygen concentration 
subplot(2,2,4)
plot(btlsum.Winkler_umolkg, mdlcal10.Residuals.raw, 'k.','Markersize',10); hold on;
ylabel({'Residual, Winkler - NLMR output','(\mumol/kg)'})
xlabel('Winkler (\mumol/kg)')
grid on
sgtitle('Irminger Year 5 Cast 10: NLMR')
...
% Only HIP Winklers so Winkler_umolkg == Winkler_HIP_umolkg
Winklers = btlsum.Winkler_umolkg; % umol/kg calculated using calibrated CTD data 
...
%Plot residuals versus pressure 
subplot(2,2,1)
plot(btlsum.prs, mdlcal21.Residuals.raw, 'k.','Markersize',10); hold on;
ylabel({'Residual, Winkler - NLMR output','(\mumol/kg)'})
xlabel('Pressure (db)')
grid on
...
%Plot residuals versus cast number 
subplot(2,2,2)
plot(datenum(btlsum.Date) - min(datenum(btlsum.Date)), mdlcal21.Residuals.raw, 'k.','Markersize',10); hold on;
ylabel({'Residual, Winkler - NLMR output','(\mumol/kg)'})
xlabel(['Days since ' datestr(min(datenum(btlsum.Date)))])
grid on
...
%Plot residuals versus temperature 
subplot(2,2,3)
plot(btlsum.t, mdlcal21.Residuals.raw, 'k.','Markersize',10); hold on;
ylabel({'Residual, Winkler - NLMR output','(\mumol/kg)'})
xlabel('Temperature (\circC)')
grid on
...
%Plot residuals versus oxygen concentration 
subplot(2,2,4)
plot(btlsum.Winkler_umolkg, mdlcal21.Residuals.raw, 'k.','Markersize',10); hold on;
ylabel({'Residual, Winkler - NLMR output','(\mumol/kg)'})
xlabel('Winkler (\mumol/kg)')
grid on
sgtitle('Irminger Year 5 Cast 21: HIP Winklers')
...
function [btlsum] = combine_btl_files(leah_btl_file,btl_file,btlsum,CTD_sen)
    ...
    if height(btlsum) ~= 1
        ...
        btlsum.Winkler_HIP_umolkg = btlsum.Winkler_HIP_mLL*1000*44.661./btlsum.prho; % HIP WInklers 
        ...
        'Winkler_OOI_umolkg','Winkler_HIP_umolkg','GSW_oxsol_umolkg','SBE_oxsol_umolkg'};

...
plotm(ooi_latlon(:,1),ooi_latlon(:,2),'^','markersize',8,'MarkerFaceColor','k',...
    'MarkerEdgeColor','k')
hold on
% plotm(cast06u.lat(1),cast06u.lon(1),'.','markersize',30,'Color',forestgreen)
% plotm(cast07u.lat(1),cast07u.lon(1),'.','markersize',30,'Color',red)
...
% plotm(cast11u.lat(1),cast11u.lon(1),'.','markersize',30,'Color','k')
plotm(cast12u.lat(1),cast12u.lon(1),'.','markersize',30,'Color',green)
plotm(cast13u.lat(1),cast13u.lon(1),'.','markersize',30,'Color',forestgreen)
plotm(cast14u.lat(1),cast14u.lon(1),'.','markersize',30,'Color',purple)
plotm(cast15u.lat(1),cast15u.lon(1),'.','markersize',30,'Color',brightpurple)
% plotm(cast16u.lat(1),cast16u.lon(1),'.','markersize',30,'Color','k')
% plotm(cast17u.lat(1),cast17u.lon(1),'.','markersize',30,'Color',yellow)
% plotm(cast18u.lat(1),cast18u.lon(1),'.','markersize',30,'Color',yellow)
plotm(cast19u.lat(1),cast19u.lon(1),'.','markersize',30,'Color',red)
plotm(cast20u.lat(1),cast20u.lon(1),'.','markersize',30,'Color',navy)
plotm(cast21u.lat(1),cast21u.lon(1),'.','markersize',30,'Color',maroon)
plotm(cast22u.lat(1),cast22u.lon(1),'.','markersize',30,'Color',yellow)
% plotm(cast23u.lat(1),cast23u.lon(1),'.','markersize',30,'Color','k')

title('Irminger 9: CTD Casts')
legend('','OOI Asset','Cast 9','Cast 12','Cast 13','Cast 14','Cast 15','Cast 19',...
    'Cast 20','Cast 21','Cast 22','Location','southoutside')

subplot(1,3,2)
plot(cast09d.DOcorr_umolkg,cast09d.pt,'Linewidth',1.2,'Color',blue)
hold on
plot(cast12d.DOcorr_umolkg,cast12d.pt,'Linewidth',1.2,'Color',red)
plot(cast13d.DOcorr_umolkg,cast13d.pt,'Linewidth',1.2,'Color',maroon)
plot(cast14d.DOcorr_umolkg,cast14d.pt,'Linewidth',1.2,'Color',purple)
plot(cast15d.DOcorr_umolkg,cast15d.pt,'Linewidth',1.2,'Color',brightpurple)
plot(cast19d.DOcorr_umolkg,cast19d.pt,'Linewidth',1.2,'Color',forestgreen)
plot(cast20d.DOcorr_umolkg,cast20d.pt,'Linewidth',1.2,'Color',navy)
plot(cast21d.DOcorr_umolkg,cast21d.pt,'Linewidth',1.2,'Color',green)
plot(cast22d.DOcorr_umolkg,cast22d.pt,'Linewidth',1.2,'Color',yellow)
ylabel('PT (\circC)')
...
subplot(1,3,3)
% plot(btlsum0.Winkler_umolkg,btlsum0.pt,'.r','markersize',20)
% hold on
plot(btlsum_yr9_OOI.Winkler_umolkg,btlsum_yr9_OOI.pt,'.k','markersize',20)
hold on
plot(cast09u.DOcorr_umolkg,cast09u.pt,'Linewidth',1.2,'Color',blue)
plot(cast12u.DOcorr_umolkg,cast12u.pt,'Linewidth',1.2,'Color',red)
plot(cast13u.DOcorr_umolkg,cast13u.pt,'Linewidth',1.2,'Color',maroon)
plot(cast14u.DOcorr_umolkg,cast14u.pt,'Linewidth',1.2,'Color',purple)
plot(cast15u.DOcorr_umolkg,cast15u.pt,'Linewidth',1.2,'Color',brightpurple)
plot(cast19u.DOcorr_umolkg,cast19u.pt,'Linewidth',1.2,'Color',forestgreen)
plot(cast20u.DOcorr_umolkg,cast20u.pt,'Linewidth',1.2,'Color',navy)
plot(cast21u.DOcorr_umolkg,cast21u.pt,'Linewidth',1.2,'Color',green)
plot(cast22u.DOcorr_umolkg,cast22u.pt,'Linewidth',1.2,'Color',yellow)
% plot(btlsum0.Winkler_umolkg,btlsum0.pt,'.r','markersize',20)
plot(btlsum_yr9_OOI.Winkler_umolkg,btlsum_yr9_OOI.pt,'.k','markersize',20)
% legend('Winklers Removed','Winklers Used','Location','NW')
legend('Winklers','Location','NW')
grid on
ylabel('PT (\circC)')
xlabel('DO (\mumol kg^-^1)')
title('Upcasts')
...
function plotmap(btlsum,upcasts,downcasts,cast_num)
    % OOI asset locations 
    ooi_latlon = [59.9341, -39.4673
    59.8177, -39.8412
    59.7155, -39.3148];
    ...
    for i = 1:length(cast_num)
        plotm(upcasts{cast_num(i)}.lat(1),upcasts{cast_num(i)}.lon(1),'.','Markersize',30)
    end
...
%     
    ...
    cn = string(cast_num);
    legend(cn(end - (length(cn)-1)),cn(end - (length(cn)-2)),cn(end - (length(cn)-3)),cn(end - (length(cn)-4)))
%     legend('Winklers','Location','NW')
    grid on
    ylabel('PT (\circC)')
    xlabel('DO (\mumol kg^-^1)')

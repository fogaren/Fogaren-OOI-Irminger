...
dir = 'G:\Shared drives\NSF_Irminger\OOI Cruises CTD Casts\CTD_Data\Alfresco\Year1';
cd(dir)
load Year1_DOcal.mat
ns = 9; % Start of cast numbers in file name
ne = 11; % End of cast numbers in file name 

dc_dir = 'C:\Users\fogaren\Documents\SBE\Year1\downcasts';
uc_dir = 'C:\Users\fogaren\Documents\SBE\Year1\upcasts';

savefile = 0; % savefile == 1 for saving; savefile == 0, don't save
bcodmo = 0; % write to csv files if == 1
bco_dmo = 'G:\Shared drives\NSF_Irminger\OOI Cruises CTD Casts\BCO-DMO Submission\KN221-04';
...
% % Calibrate whole cruise with Winklers on cast 6
cal6 = cal;
cal6.SOCcalc = cal.SOCcalc6;
cal6.Ecalc = cal.Ecalc6;
cal6.gain = cal.gain6;
cal6.Winkler_outliers = cal.Winkler_outliers6;
% 
% btlsum_tbl = calibrate_CTD_oxygen(btlsum_tbl,cal6,SOC_type);
...
for i = 1:length(cast_num)
        ...
        downcasts{cast_num(i)} = process_cast_noCTDcal(mydowncast{cast_num(i)}, CTD_sen, cal6, SOC_type, CruiseStartTime,cast_num(i));
        upcasts{cast_num(i)} = process_cast_noCTDcal( myupcast{cast_num(i)}, CTD_sen, cal6, SOC_type, CruiseStartTime,cast_num(i));
end
...
bad_casts = [NaN; % bad downcasts 
    NaN]'; % bad upcasts 
plot_calibrated_pTemp(downcasts,upcasts,cast_num,btlsum_tbl,cal,bad_casts,'KN221-04 OOI Year 1')
...
for i = 1:length(cast_num)
    ...
    if height(btlsum{cast_num(i)}) > 0
        plot(btlsum{cast_num(i)}.Discrete_Salinity1_psu,btlsum{cast_num(i)}.prs,'.k','MarkerSize',20)
        plot(btlsum{cast_num(i)}.Discrete_Salinity2_psu,btlsum{cast_num(i)}.prs,'.k','MarkerSize',20)
    end

    axis ij
    grid on
    title(['KN221-04 Cast ' num2str(cast_num(i))])
end
%%
figure
plot(btlsum_tbl.prs,btlsum_tbl.Discrete_Salinity1_psu-btlsum_tbl.sal1,'.','MarkerSize',20)
hold on
plot(btlsum_tbl.prs,btlsum_tbl.Discrete_Salinity1_psu-btlsum_tbl.sal2,'.','MarkerSize',20)
nanmean([btlsum_tbl.Discrete_Salinity1_psu-btlsum_tbl.sal1; btlsum_tbl.Discrete_Salinity2_psu-btlsum_tbl.sal1])
nanstd([btlsum_tbl.Discrete_Salinity1_psu-btlsum_tbl.sal1; btlsum_tbl.Discrete_Salinity2_psu-btlsum_tbl.sal1])
nanmean([btlsum_tbl.Discrete_Salinity1_psu-btlsum_tbl.sal2; btlsum_tbl.Discrete_Salinity2_psu-btlsum_tbl.sal2])
nanstd([btlsum_tbl.Discrete_Salinity1_psu-btlsum_tbl.sal2; btlsum_tbl.Discrete_Salinity2_psu-btlsum_tbl.sal2])
%% Save processed data 
if savefile == 1

    btlsum_yr1 = btlsum;
    btlsum_tbl_yr1 = btlsum_tbl; 
    upcasts_yr1 = upcasts;
    downcasts_yr1 = downcasts;
    btl_num_yr1 = btl_num;
    cast_num_yr1 = cast_num;
    SBE_cal_yr1 = cal;
    dt_KF_Processed_yr1 = datetime('now');
    clear btlsum_tbl
    cd(dir)
%     cd('G:\Shared drives\NSF_Irminger\OOI Cruises CTD Casts\CTD_Data\Processed')
    save Year1_Processed_KF.mat upcasts_* downcasts_* btl_num_* cast_num_* SBE_cal_* btlsum_* dt_KF_Processed_*
end
%%
...
for i = 1:length(cast_num)
    ...
    temp_flag = ones(size(dwn_out.t))*1;
    sal_flag = ones(size(dwn_out.t))*1;
    oxycur_flag = ones(size(dwn_out.t))*3;
    ctdoxy_flag = ones(size(dwn_out.t))*3;

    fheader = ['KN221-04    Calibrated Oxygen Downcast   Station: ' num2str(cast_num(i)) newline...
    ...
    fileIDd = fopen(['KN221-04_' sprintf('%03d',cast_num(i)) 'd.csv'],'w');
...
%     btl_out.Winkler3_umolkg(isnan(btl_out.Winkler3_umolkg)) = -999;
    ...
    temp_flag = ones(size(btl_out.t))*1;
    ...
    if index1 + index2 == 2
        fheader = ['KN221-04    Post-CTD Oxygen Calibration   Station: ' num2str(btl_num(i)) newline...
    ...
    fileID = fopen(['KN221-04_' sprintf('%03d',btl_num(i)) 'btl.csv'],'w');
...
plot(btlsum.Winkler1_umolkg(btlsum.Cast == 6),btlsum.pt(btlsum.Cast == 6),'.k','markersize',20)
ylabel('PT (\circC)')
xlabel('DO (\mumol kg^-^1)')
title('Upcasts')
grid on
...
plot(btlsum.Winkler1_umolkg,btlsum.pt,'.','markersize',20,'Color',yellow)
plot(btlsum.Winkler2_umolkg,btlsum.pt,'.','markersize',20,'Color',yellow)

...
cal0 = cal;
%%
temp_threshold = -88; % remove Winklers less than this temp 
...
Winkelrs_to_use(X(:,3) < temp_threshold) = NaN;
...
%  Sensor 2, Group 1: Stations 1-36 SOC cn
...
btl_check = [btlsum_tbl.Bottle; btlsum_tbl.Bottle];
Winklers_to_use = [btlsum_tbl.Winkler1_umolkg; btlsum_tbl.Winkler2_umolkg];
Winklers_to_use(X(:,3) < temp_threshold) = NaN;
pdens = [btlsum_tbl.prho; btlsum_tbl.prho];
...
%% Sensor 2, Group 2: Stations 37 thru end with SOCk
...
xlabel('Winkler DO (\mumol/kg)')
grid on
sgtitle(['AR84-02 Group 2: SOC_k CTD Sensor ' num2str(CTD_sen)])

%%
sgtitle('AR84-02 CTD-DO Package 2: All fits')

%% Pick back up here and make sure called function works for duplicate CTD packages 
...
%%
cal = cal2; SOC_type = 1; %%% The line to change 
cast_num = cal.casts;
dummy = [];
clear dummy1
for j = 1:length(cast_num)

...
[ind,~] = find(btlsum.Cast < 9); 
Winklers = btlsum.Winkler_umolkg; % umol/kg calculated using calibrated CTD data 
Winklers(ind) = NaN; % Removes data associated with bad CTD data 
...
subplot(2,2,4)
plot(btlsum.oxsol_umolkg, mdl.Residuals.raw, 'k.','Markersize',10); hold on;
ylabel({'Residual, Winkler - NLMR output','(\mumol/kg)'})
xlabel('Winkler (\mumol/kg)')
grid on
sgtitle('Irminger Year 3: NLMR')
%%
...
xlabel('Cast number')
grid on
text(min(cn)+2,max(SOCdt)+0.0005,{['SOC = ' num2str(b_cn(1),6) ' + ' num2str(b_cn(2)) '*cast number' ],...
...
xlabel(['Days since ' datestr(min(datenum(btlsum.Date)))])
grid on
text(min(dtx)+2,max(SOCdt)+0.0005,{['SOC = ' num2str(b_dt(1),6) ' + ' num2str(b_dt(2)) '*time (days)' ],...
    ['R-squared = ' num2str(SOClm_dt.Rsquared.Ordinary)]})
title('By time (days)')
legend('Location','SE')
sgtitle('Irminger 3: SOC drift')
%% Calculate drift with variable SOC with time 
btlsum.dt = datenum(btlsum.Date) - datenum(2017,08,05,20,52,20); % minus first cast time

...
% 
% load AR8402_DOcal_nohyst.mat
% dc_dir = 'C:\Users\fogaren\Desktop\Irminger_2024\AR84_02\CTDdata\no_hyst\downcasts';
% uc_dir = 'C:\Users\fogaren\Desktop\Irminger_2024\AR84_02\CTDdata\no_hyst\upcasts';
...
% cal1 = cal1_defaultH; cal2 = cal2_defaultH; cal3 = cal3_defaultH; 
% 
% CruiseStartTime = mydowncast{cast_num(1)}.StartTimeUTC(1); % Needed for variable SOC 

% btlsum_tbl = btlsum_tbl_AR8402_nohyst;
% CTD_sen = btlsum_tbl.CTD_sen(1); % Sensor package to use for calibration; same as bottle processing 
% cal1 = cal1_nohyst; cal2 = cal2_nohyst; cal3 = cal3_nohyst; 
% 
% CruiseStartTime = mydowncast{cast_num(1)}.StartTimeUTC(1); % Needed for variable SOC 

btlsum_tbl = btlsum_tbl_AR8402_custH;
CTD_sen = btlsum_tbl.CTD_sen(1); % Sensor package to use for calibration; same as bottle processing 
cal1 = cal1_custH; cal2 = cal2_custH; cal3 = cal3_custH; 
...
%%
% Casts cal3 = 224:268
cal3.casts = 224:268; % add the 2 casts beyond the last Winkler cast 
SOC_type = 1; % SOC_k
for j = 1:length(cal3.casts)
        btlsum{cal3.casts(j)} = btlsum_tbl(btlsum_tbl.Cast == cal3.casts(j),:); 
        downcasts{cal3.casts(j)} = process_cast(dcc{cal3.casts(j)}, mydowncast{cal3.casts(j)}, CTD_sen, cal3, SOC_type, CruiseStartTime);
        upcasts{cal3.casts(j)} = process_cast(ucc{cal3.casts(j)}, myupcast{cal3.casts(j)}, CTD_sen, cal3, SOC_type, CruiseStartTime);
end
%%
% btlsum_nohyst = btlsum;
% downcasts_nohyst = downcasts;
% upcasts_nohyst = upcasts; 
% % 
%%
btlsum_cust = btlsum;
...
% %
% btlsum_defaultH = btlsum;
% downcasts_defaultH = downcasts;
% upcasts_defaultH = upcasts; 

%% Plot final data
cast_num_plot = 1:28;
for j = 1:length(cast_num_plot)
    plot_calibrated_DO(downcasts_defaultH{cast_num_plot(j)},upcasts_defaultH{cast_num_plot(j)},btlsum_defaultH{cast_num_plot(j)},downcasts_cust{cast_num_plot(j)},upcasts_cust{cast_num_plot(j)},downcasts_nohyst{cast_num_plot(j)},upcasts_nohyst{cast_num_plot(j)})
end
%%
...
function plot_calibrated_DO(downcasts_default,upcasts_default,btlsum,downcasts_cust,upcasts_cust,downcasts_nohyst,upcasts_nohyst)
...
navy = [0.078431     0.16863     0.54902];
    if height(btlsum) == 1 
        ...
        figure
        ax1 = subplot(1,2,1);
        plot(downcasts_default.pt,downcasts_default.prs,'Linewidth',1.2,'Color',grey)
        hold on
        plot(upcasts_default.pt,upcasts_default.prs,'Linewidth',1.2,'Color','k')
        axis ij
        ylabel('Pressure (db)')
        ...
        ax2 = subplot(1,2,2);
        plot(downcasts_default.DOcorr_umolkg,downcasts_default.prs,'Linewidth',1.2,'Color',grey)
        hold on
        plot(downcasts_cust.DOcorr_umolkg,downcasts_cust.prs,'Linewidth',1.2,'Color',blue)
        plot(downcasts_nohyst.DOcorr_umolkg,downcasts_nohyst.prs,'Linewidth',1.2,'Color',red)
        plot(upcasts_default.DOcorr_umolkg,upcasts_default.prs,'Linewidth',1.2,'Color','k')    
        plot(upcasts_cust.DOcorr_umolkg,upcasts_cust.prs,'Linewidth',1.2,'Color',navy)
        plot(upcasts_nohyst.DOcorr_umolkg,upcasts_nohyst.prs,'Linewidth',1.2,'Color',maroon)
        axis ij
        ylabel('Pressure (db)')
        xlabel('Oxygen (\mumol kg^-^1)')
        ax = gca; grid on
        ax.XAxisLocation = 'top';
        sgtitle(['Cast ' num2str(downcasts_default.Station(1))])
        linkaxes([ax2 ax1],'y')
        legend('Default','Custom','None','Location','SW')
    end

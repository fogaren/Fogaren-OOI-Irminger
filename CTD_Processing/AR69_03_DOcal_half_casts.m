...
Winkler_file = 'AR69_03_winklers_finalized_KF_for_sensitivity_analysis';
% Sheet == Run 1 is Winkler 1 + 2 for ~every other CTD cast removed
% Sheet == Run 2 is Winkler1 for casts deeper than 1000 m 
% Sheet == Run 3 is Winklers 1 for all Casts 
% Sheet == Run 4 is Winklers 2 for all Casts 
...
grid on
ylim([-4 4])
sgtitle('AR69-03: SOC_k Winkler 2 All Casts')
...
%% For calibration using just one group
cal = cal_all; SOC_type = 1; %%% The line to change 
cast_num = cal.casts;
dummy = [];
for i = 1:length(Wink_casts)
    dummy1 = btlsum{Wink_casts(i)};
    dummy = [dummy; dummy1];
end
...
btlsum_tbl_Wink1 = btlsum_tbl; 
btlsum_Wink1 = btlsum; 
clear cal btlsum btlsum_tbl
% cd(samp_dir)
save AR6903_DO_calWink1.mat btlsum_* cal*

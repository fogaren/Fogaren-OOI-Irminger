...
% No calibrated salinity data for this cruise? 
cd(cal_dir)
...
for i = 1:height(mybtlcasts) % Number of bottle summary files 
    btlsum{i} = process_btl_files(mybtlfiles(i,:),CTD_sen) ;
%     btlsum{i} = process_btl_files(mybtlfiles(i,:),Winklers(Winklers.Cast == btlcasts(i),:),CTD_sen) ;
end
...
function btlsum = process_btl_files(my_btl_file,CTD_sen)
%btlsum = process_btl_files(my_btl_file,Winkler_table,CTD_sen)
...
%     btlsum.Properties.VariableNames = {'Cruise','Cast','Bottle','Winkler_mLL'};
    ...
    vars = {'Bottle','Date','PrDM','DepSM','T090C','T190C','C0mS_cm','C1mS_cm','Latitude','Longitude','Sbeox0V'};
    btl = btl(:,vars);
    btl.Properties.VariableNames = {'Bottle','Date','prs','depth','temp1','temp2','cond1','cond2','lat','lon','oxy_volts'};
    btl.CTDcal(:) = {'False'};
...
%     btlvars = {'Cruise','Date','Cast','Bottle','prs','depth','lat','lon','temp1','temp2','sal1','sal2','CTDcal',...
%     'CTD_sen','t','CT','pt','SP','SA','rho','prho','sigma0','oxy_volts','Winkler_mLL','Winkler_umolkg','O2sol_umolkg'};
%     btlsum = btlsum(:,btlvars);
btlvars = {'Date','Bottle','prs','depth','lat','lon','temp1','temp2','sal1','sal2','CTDcal',...
    'CTD_sen','t','CT','pt','SP','SA','rho','prho','sigma0','oxy_volts','O2sol_umolkg'};
    btlsum = btlsum(:,btlvars);
end
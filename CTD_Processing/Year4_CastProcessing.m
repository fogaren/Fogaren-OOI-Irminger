...
dir = 'G:\Shared drives\NSF_Irminger\OOI Cruises CTD Casts\CTD_Data\Alfresco\Year4';
cd(dir)
load Year4_DOcal.mat
ns = 7; % Start of cast numbers in file name
ne = 9; % End of cast numbers in file name 

dc_dir = 'C:\Users\fogaren\Documents\SBE\Year4\downcasts';
uc_dir = 'C:\Users\fogaren\Documents\SBE\Year4\upcasts';
...
%%
bad_casts = [1 2 3 4 5 6 7 8 ; % bad downcasts 
    1 2 3 4 5 6 7 8]'; % bad upcasts 
plot_calibrated_pTemp(downcasts,upcasts,cast_num,btlsum_tbl,cal,bad_casts,'AR21 OOI Year 4')
...
for i = 1:length(cast_num)
    ...
    axis ij
    grid on
    title(['AR21 Cast ' num2str(cast_num(i))])
end
%%

sal1sorted = (abs(btlsum_tbl.Discrete_Salinity_psu(btlsum_tbl.Cast ~= 11)-btlsum_tbl.sal1(btlsum_tbl.Cast ~= 11)))
sal2sorted = (abs(btlsum_tbl.Discrete_Salinity_psu(btlsum_tbl.Cast ~= 11)-btlsum_tbl.sal2(btlsum_tbl.Cast ~= 11)))
...
figure
plot(sal1sorted,'.','MarkerSize',20)
hold on
plot(sal2sorted,'.','MarkerSize',20)
%% Save processed data 
if savefile == 1

    btlsum_yr4 = btlsum;
    btlsum_tbl_yr4 = btlsum_tbl; 
    upcasts_yr4 = upcasts;
    downcasts_yr4 = downcasts;
    btl_num_yr4 = btl_num;
    cast_num_yr4 = cast_num;
    cal_yr4 = cal;
    dt_Processed_yr4 = datetime('now');
...
%     cd('G:\Shared drives\NSF_Irminger\OOI Cruises CTD Casts\CTD_Data\Processed')
    save Year4_Processed_KF.mat upcasts_yr* downcasts_yr* btl_num_yr* cast_num_yr* cal_yr* btlsum_yr* btlsum_tbl_yr* dt_Processed_yr*
end
%%
...
for i = 1:length(cast_num)
    ...
    if cast_num(i) < 9
        ...
        oxycur_flag = ones(size(dwn_out.t))*3; % Questionable  
    ...
    fheader = ['AR21    Calibrated Oxygen Downcast   Station: ' num2str(cast_num(i)) newline...
    ...
    fileIDd = fopen(['AR21_' sprintf('%03d',cast_num(i)) 'd.csv'],'w');
...
for i = 1:length(cast_num)
    ...
    if cast_num(i) < 9
        ...
        oxycur_flag = ones(size(up_out.t))*3; % Questionable  
    ...
    fheader = ['AR21    Calibrated Oxygen Upcast   Station: ' num2str(cast_num(i)) newline...
    'Latitude: ' sprintf('%.4f',up_out.lat(1)) '   Longitude: ' sprintf('%.4f',up_out.lon(1))...
    '   ' datestr(up_out.StartTimeUTC(1)) newline...
    sprintf('CTDPRES, CTDTEMP_ITS90, CTDTEMP_flag, CTDSAL_PSS78, CTDSAL_flag, CTDOXYCUR, CTDOXYCUR_flag, CTDOXY, CTDOXY_flag') newline...
    sprintf('dbar, deg_C, n.a., n.a., n.a., volts, n.a., umol/kg, n.a.') newline]; 

    fileIDu = fopen(['AR21_' sprintf('%03d',cast_num(i)) 'u.csv'],'w');
...
plot(btlsum.Winkler_umolkg(btlsum.NLMR_Outlier == 1),btlsum.pt(btlsum.NLMR_Outlier == 1),'.','markersize',20,'Color',yellow)

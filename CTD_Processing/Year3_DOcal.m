...
% correction 
btl_dir = 'G:\Shared drives\NSF_Irminger\OOI Cruises CTD Casts\CTD_Data\Alfresco\Bottle_Files\Year3';
samp_dir = 'G:\Shared drives\NSF_Irminger\OOI Cruises CTD Casts\CTD_Data\Alfresco\Year3'; % Winkler file location 
Winkler_file = 'Irminger_Sea-03_AR7-01_Discrete_Summary_KF.xlsx'; % Winkler file name 
...
btlcasts = str2num(mybtlfiles(:,7:9)); % Pulls out cast numbers that have bottle files 
...
% cast_ind = btlsum_tbl.Cast == 7 ;
% Winklers_to_use = btlsum_tbl.Winkler_umolkg; 
% Winklers_to_use(cast_ind == 0) = NaN; % 1 = remove, 0 = include
...
title('By Cast')
sgtitle('Year 3 AR07-01: SOC Drift')
...
dt = datenum(btlsum_tbl.Date) - datenum(2016,07,09,12,37,09); % minus time of cast 5 7/9/2016  12:37:09 PM
X_dt = [btlsum_tbl.oxy_volts,btlsum_tbl.O2sol_umolkg,btlsum_tbl.t,btlsum_tbl.prs,dt];
Winklers_to_use = btlsum_tbl.Winkler_umolkg; 

% % Remove these Winklers for regression 
cast_ind = btlsum_tbl.Cast ==5 | btlsum_tbl.Cast == 6;
Winklers_to_use = btlsum_tbl.Winkler_umolkg; 
...
mdlcal_dt1 = mdl_dt5;
% cal for SOC_dt 
cal.SOCcalc_dt1 = mdlcal_dt1.Coefficients.Estimate(1);
cal.Ecalc_dt1 = mdlcal_dt1.Coefficients.Estimate(2);
cal.SOCrate_dt1 = mdlcal_dt1.Coefficients.Estimate(3);
cal.Winkler_outliers_dt1 = Winkler_outliers4_dt;


%% Calculate drift with variable SOC with time 
dt = datenum(btlsum_tbl.Date) - datenum(2016,07,11,20,47,04); % 11-Jul-2016 20:47:04 Cast 7 first time 
% dt = datenum(btlsum_tbl.Date) - datenum(2016,07,09,12,37,09);
X_dt = [btlsum_tbl.oxy_volts,btlsum_tbl.O2sol_umolkg,btlsum_tbl.t,btlsum_tbl.prs,dt];
Winklers_to_use = btlsum_tbl.Winkler_umolkg; 

% % Include these Winklers for regression 
cast_ind = btlsum_tbl.Cast ==5 | btlsum_tbl.Cast == 6;
Winklers_to_use = btlsum_tbl.Winkler_umolkg; 
Winklers_to_use(cast_ind == 1) = NaN; % 0 = include; 1 = exclude  
...
mdlcal_dt2 = mdl_dt5;
% cal for SOC_dt 
cal.SOCcalc_dt2 = mdlcal_dt2.Coefficients.Estimate(1);
cal.Ecalc_dt2 = mdlcal_dt2.Coefficients.Estimate(2);
cal.SOCrate_dt2 = mdlcal_dt2.Coefficients.Estimate(3);
cal.Winkler_outliers_dt2 = Winkler_outliers4_dt;
%%
f = figure;
f.Position = [100 100 840 500];
%Plot residuals versus pressure 
subplot(2,2,1)
plot(btlsum_tbl.prs, mdlcal_dt1.Residuals.raw, '.','Markersize',20); hold on;
plot(btlsum_tbl.prs, mdlcal_dt2.Residuals.raw, '.k','Markersize',20); 
ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
xlabel('Pressure (db)')
legend('Fit 1','Fit 2','Location','NE','Orientation','horizontal')
grid on

%Plot residuals versus cast number/time 
subplot(2,2,2)
plot(btlsum_tbl.Cast, mdlcal_dt1.Residuals.raw, '.','Markersize',20); hold on;
plot(btlsum_tbl.Cast, mdlcal_dt2.Residuals.raw, '.k','Markersize',20);
ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
xlabel('Station Number')
grid on
...
%Plot residuals versus temperature 
subplot(2,2,3)
plot(btlsum_tbl.t, mdlcal_dt1.Residuals.raw, '.','Markersize',20); hold on;
plot(btlsum_tbl.t, mdlcal_dt2.Residuals.raw, '.k','Markersize',20);
ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
xlabel('Temperature (\circC)')
grid on
...
%Plot residuals versus oxygen concentration 
subplot(2,2,4)
plot(btlsum_tbl.Winkler_umolkg, mdlcal_dt1.Residuals.raw, '.','Markersize',20); hold on;
plot(btlsum_tbl.Winkler_umolkg, mdlcal_dt2.Residuals.raw, '.k','Markersize',20);
ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
xlabel('Winkler DO (\mumol/kg)')
grid on
sgtitle('Year 3 AR07-01: SOC_d_t')
...
cal.SOCcalc_cn1 = mdlcal_cn1.Coefficients.Estimate(1);
cal.Ecalc_cn1 = mdlcal_cn1.Coefficients.Estimate(2);
cal.SOCrate_cn1 = mdlcal_cn1.Coefficients.Estimate(3);
cal.Winkler_outliers_cn1 = Winkler_outliers4_cn;
%% Calculate SOC that varies by station 
X_cn = [btlsum_tbl.oxy_volts,btlsum_tbl.O2sol_umolkg,btlsum_tbl.t,btlsum_tbl.prs,btlsum_tbl.Cast];
...
mdlcal_cn2 = mdl_cn5;
% cal.Winkler_outliers_cn = Winkler_outliers4_cn;
cal.SOCcalc_cn2 = mdlcal_cn2.Coefficients.Estimate(1);
cal.Ecalc_cn2 = mdlcal_cn2.Coefficients.Estimate(2);
cal.SOCrate_cn2 = mdlcal_cn2.Coefficients.Estimate(3);
cal.Winkler_outliers_cn2 = Winkler_outliers4_cn;
%%
f = figure;
f.Position = [100 100 840 500];
%Plot residuals versus pressure 
subplot(2,2,1)
plot(btlsum_tbl.prs, mdlcal_cn1.Residuals.raw, '.k','Markersize',20); hold on;
plot(btlsum_tbl.prs, mdlcal_cn2.Residuals.raw, '.k','Markersize',20);
ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
xlabel('Pressure (db)')
grid on
...
%Plot residuals versus cast number/time 
subplot(2,2,2)
plot(btlsum_tbl.Cast, mdlcal_cn1.Residuals.raw, '.k','Markersize',20); hold on;
plot(btlsum_tbl.Cast, mdlcal_cn2.Residuals.raw, '.k','Markersize',20);
ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
xlabel('Station Number')
grid on
...
%Plot residuals versus temperature 
subplot(2,2,3)
plot(btlsum_tbl.t, mdlcal_cn1.Residuals.raw, '.k','Markersize',20); hold on;
plot(btlsum_tbl.t, mdlcal_cn2.Residuals.raw, '.k','Markersize',20);
ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
xlabel('Temperature (\circC)')
grid on
...
%Plot residuals versus oxygen concentration 
subplot(2,2,4)
plot(btlsum_tbl.Winkler_umolkg, mdlcal_cn1.Residuals.raw, '.k','Markersize',20); hold on;
plot(btlsum_tbl.Winkler_umolkg, mdlcal_cn2.Residuals.raw, '.k','Markersize',20);
ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
xlabel('Winkler DO (\mumol/kg)')
grid on
sgtitle('Year 3 AR07-01: SOC_c_n')

%% Decided on SOC calibration 
SOC_type = 2; % variable by time 
% SOC_type % 1 = constant, % 2 = changes as a function of cruise time % 3 = changes as a function of station number 
Winklers_questionable = [cal.Winkler_outliers_dt1; cal.Winkler_outliers_dt2]; 
...
cal1.SOCcalc_dt = cal.SOCcalc_dt1;
cal1.Ecalc_dt = cal.Ecalc_dt1;
cal1.SOCrate_dt = cal.SOCrate_dt1;

cal2.SOCcalc_dt = cal.SOCcalc_dt2;
cal2.Ecalc_dt = cal.Ecalc_dt2;
cal2.SOCrate_dt = cal.SOCrate_dt2;
% 
% cal1.SOCcalc_dt = cal.SOCcalc_cn1;
% cal1.Ecalc_dt = cal.Ecalc_cn1;
% cal1.SOCrate_dt = cal.SOCrate_cn1;
% 
% cal2.SOCcalc_dt = cal.SOCcalc_cn2;
% cal2.Ecalc_dt = cal.Ecalc_cn2;
% cal2.SOCrate_dt = cal.SOCrate_cn2;

btlsum_tbl.DOcorr_umolkg = NaN(height(btlsum_tbl),1);

dt1 = datenum(btlsum_tbl.Date) - datenum(2016,07,09,12,37,09); % 7/9/2016  12:37:09 PM
x1 = [btlsum_tbl.oxy_volts,btlsum_tbl.O2sol_umolkg,btlsum_tbl.t,btlsum_tbl.prs,dt1];

% DO casts 5 and 6 with calibration fit 1  
ind1 = find(btlsum_tbl.Cast == 5 | btlsum_tbl.Cast == 6);        
        % SBE functional form with SOC as a function of cruise time
        btlsum_tbl.DOcorr_umolkg(ind1) = ((cal1.SOCrate_dt*x1(ind1,5)) + cal1.SOCcalc_dt*(x1(ind1,1) + cal1.VOFFSET)).*x(ind1,2)...
            .*(1 + cal1.A*x1(ind1,3) + cal1.B*x1(ind1,3).^2 + cal1.C*x1(ind1,3).^3)...
            .*exp((cal1.Ecalc_dt*x1(ind1,4))./(x1(ind1,3) + 273.15));

dt2 = datenum(btlsum_tbl.Date) - datenum(2016,07,11,20,47,04); % 11-Jul-2016 20:47:04 Cast 7 first time 
x2 = [btlsum_tbl.oxy_volts,btlsum_tbl.O2sol_umolkg,btlsum_tbl.t,btlsum_tbl.prs,dt2];

% DO Other casts with calibration fit 2  
ind2 = find(btlsum_tbl.Cast == 7 | btlsum_tbl.Cast == 8 | btlsum_tbl.Cast == 9 | btlsum_tbl.Cast == 10);
        btlsum_tbl.DOcorr_umolkg(ind2) = ((cal2.SOCrate_dt*x2(ind2,5)) + cal2.SOCcalc_dt*(x2(ind2,1) + cal2.VOFFSET)).*x(ind2,2)...
            .*(1 + cal2.A*x2(ind2,3) + cal2.B*x2(ind2,3).^2 + cal2.C*x2(ind2,3).^3)...
            .*exp((cal2.Ecalc_dt*x2(ind2,4))./(x2(ind2,3) + 273.15));
...
%     btlsum0 = join(leah_btl,btl,'Keys','Bottle');
    btlsum = join(btl,btlsum,'Keys','Bottle');  

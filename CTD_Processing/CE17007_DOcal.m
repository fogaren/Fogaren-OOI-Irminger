...
% correction 
btl_dir = 'C:\Users\fogaren\Documents\Python\Bottle_Files\CE17007';
cal_dir = '';
...
% x = [btlsum_tbl.oxy_volts,btlsum_tbl.SBE_Oxsol,btlsum_tbl.t,btlsum_tbl.prs];
...
%% Looking at unique calibration for each profile 
for j = 1:length(btlcasts)
    ...
    % X = [btlsum{btlcasts(j)}.oxy_volts,btlsum{btlcasts(j)}.SBE_Oxsol,btlsum{btlcasts(j)}.t,btlsum{btlcasts(j)}.prs];
    ...
    if ~isnan(max(Winklers_to_use))
        ...
        modelfun = @(b,x)(b(1)*(x(:,1) + cal.VOFFSET)).*x(:,2)...
        .*(1 + cal.A*x(:,3) + cal.B*x(:,3).^2 + cal.C*x(:,3).^3)...
        .*exp((cal.Epicked*x(:,4))./(x(:,3) + 273.15));
        ...
        %Plot residuals versus pressure, time, station, DO concentration with outliers removed   
        mdlcal_k = mdl7;
        cal_out{btlcasts(j)}.SOCcalc = mdlcal_k.Coefficients.Estimate(1);
        % cal_out{btlcasts(j)}.Ecalc = mdlcal_k.Coefficients.Estimate(2);
        cal_out{btlcasts(j)}.gain = cal_out{btlcasts(j)}.SOCcalc/cal.SOC;
        cal_out{btlcasts(j)}.Winkler_outliers = Winkler_outliers6;
        cal_out{btlcasts(j)}.RMSE = mdlcal_k.RMSE;
        ...
        % plot(btlsum{btlcasts(j)}.prs, Winklers_to_use - DOuncorr_umolkg, '.','Markersize',20)
        ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
        xlabel('Pressure (db)')
        grid on
        ...
        % plot(btlsum{btlcasts(j)}.Cast, Winklers_to_use - DOuncorr_umolkg, '.','Markersize',20)
        ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
        xlabel('Station Number')
        grid on
        ...
        % plot(btlsum{btlcasts(j)}.t, Winklers_to_use - DOuncorr_umolkg, '.','Markersize',20)
        ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
        xlabel('Temperature (\circC)')
        grid on
        ...
        % plot(btlsum{btlcasts(j)}.Winkler_umolkg, Winklers_to_use - DOuncorr_umolkg, '.','Markersize',20)
        ylabel({'Residual, Winkler - Model','(\mumol/kg)'})
        ...
        figure(100)
        scatter(btlsum{btlcasts(j)}.prs, mdlcal_k.Residuals.raw,30,btlsum{btlcasts(j)}.t,'filled')
        hold on
        cmocean('thermal')
        box on
...
%%
for j = 4:length(btlcasts)
    figure(1)
    subplot(1,2,1)
    try
    plot(btlcasts(j),cal_out{btlcasts(j)}.gain,'.','MarkerSize',20)
    end
    hold on
    ...
    % subplot(1,3,2)
    % try
    % plot(btlcasts(j),cal_out{btlcasts(j)}.Ecalc,'.','MarkerSize',20)
    % end
    % hold on
    % title('E term')
    ...
    subplot(1,2,2)
    try
    plot(btlcasts(j),cal_out{btlcasts(j)}.RMSE,'.','MarkerSize',20)
    end
    hold on
    title('RMSE')
    xlabel('Cast Number')
...
function [btlsum] = combine_btl_files_noCTDcal(btl_file,btlsum,CTD_sen)
    ...
    vars = {'Bottle','Date','PrDM','DepSM','Latitude','Longitude','T090C','T190C','C0mS_cm','C1mS_cm','Sbeox0V','OxsolMm_Kg'};
    btl = btl(:,vars);
    btl.Properties.VariableNames = {'Bottle','Date','prs','depth','lat','lon','temp1','temp2','cond1','cond2','oxy_volts','SBE_Oxsol'};
    btl.CTDcal(:) = {'False'};
...
%     btlsum0 = join(leah_btl,btl,'Keys','Bottle');
    ...
    if height(btlsum) ~= 1
        ...
        btlsum.O2sol_umolkg = gsw_O2sol_SP_pt(btlsum.SP,btlsum.pt);
        ...
        btlsum.Winkler_umolkg = btlsum.Winkler_umolL*1000./btlsum.prho; % Winklers 
    
        % Reorder variables and remove unnecessary ones
        btlvars = {'Date','Cast','Bottle','prs','depth','lat','lon','temp1','temp2','sal1','sal2','Discrete_salinity','CTDcal',...
        'CTD_sen','t','CT','pt','SP','SA','rho','prho','sigma0','oxy_volts','SBE_Oxsol','Winkler_umolL',...
...
x = [btlsum.oxy_volts,btlsum.O2sol_umolkg,btlsum.t,btlsum.prs];
    ...
    if SOC_type == 3 % SOC varies with cast number 
        ...
        btlvars = {'Cruise','Date','Cast','Bottle','prs','depth','lat','lon','temp1','temp2','sal1','sal2','Discrete_Salinity_psu',...
            'CTDcal','CTD_sen','t','CT','pt','SP','SA','rho','prho','sigma0','oxy_volts','O2sol_umolkg','SBE_Oxsol','Winkler_mLL',...
